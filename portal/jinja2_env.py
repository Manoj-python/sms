...
def environment(**options):
    ...
    env.filters["dmy"] = dmy
    # A pure settings lookup, not per-request context — exposed as a global
    # rather than threaded through every render() call that touches
    # pay_form.html (there are several: pay.html's include, plus every
    # error/re-render branch in payments.py).
    env.globals["phonepe_enabled"] = lambda: get_settings().phonepe_enabled
    return env
