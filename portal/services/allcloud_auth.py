...
class AllCloudAuth:
    ...
    async def call_lms(
        ...
        `retry_on_5xx=False`: for the multi-tenant "does this record belong
        to this tenant" probes in multi_lms.py — confirmed live (2026-07-22)
        that AllCloud returns a plain HTTP 500 (not 404) for GetCustomerSearch/
        GetLoanAgreementNoAsync when the record isn't in that tenant, which
        is a DETERMINISTIC miss, not a transient failure, so the normal
        retry-with-backoff (up to ~1.5s of pure sleep per dead-end tenant)
        is wasted time, not resilience — it was the dominant cost in a
        7-second login for a loan whose tenant happened to be checked last.
        Transport errors/timeouts still retry regardless — those genuinely
        can be transient. Every other call site keeps the default (retry
        everything), unaffected.

        `timeout_seconds`: overrides the client's default 10s timeout for
        this one call — same multi-tenant probes as above. Every real
        discovery response (hit or miss) has taken under 1.5s in testing;
        bounding the worst case at a few seconds means a single tenant
        that's genuinely hanging (not just erroring) can't stall a login
        by the full 10s while the other two tenants may have already
        answered. Every other call site keeps the client default."""
        ...
        for attempt in range(self.settings.lms_max_retries + 1):
            ...
            try:
                ...
                request_kwargs = {} if timeout_seconds is None else {"timeout": timeout_seconds}
                if static_auth_header and body is not None:
                    headers["Content-Type"] = "application/json"
                    resp = await self._client.request(
                        method, url, headers=headers, json=body, **request_kwargs
                    )
                elif minified is not None:
                    headers["Content-Type"] = "application/json"
                    resp = await self._client.request(
                        method, url, headers=headers, content=minified.encode("utf-8"), **request_kwargs
                    )
                else:
                    resp = await self._client.request(method, url, headers=headers, **request_kwargs)
                ...
                if status >= 500 and retry_on_5xx and attempt < self.settings.lms_max_retries:
