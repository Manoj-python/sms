...
async def get_owned_txn_by_key(idempotency_key: str, session_id: str) -> PgTransaction | None:
    """Same IDOR posture as get_owned_txn, keyed by idempotency_key instead
    of the numeric pk — used by the PhonePe return page, since that's the
    id PhonePe knows the transaction by (merchant_order_id)."""
    txn = await PgTransaction.objects.filter(idempotency_key=idempotency_key).afirst()
    if txn is None or txn.session_id != session_id:
        return None
    return txn


async def get_txn_by_key(idempotency_key: str) -> PgTransaction | None:
    """Unscoped lookup for the PhonePe webhook — PhonePe's server has no
    portal session to check ownership against; the webhook's own signature
    validation (phonepe_client.validate_webhook) is what authorizes this."""
    return await PgTransaction.objects.filter(idempotency_key=idempotency_key).afirst()
...
async def confirm_phonepe_payment(txn: PgTransaction, transaction_id: str) -> PgTransaction:
    """Unlike confirm_gateway_payment (AllCloud's optimistic terminal
    state), this is only ever called once PhonePe has genuinely confirmed
    the payment (webhook event or a COMPLETED status-check) — so it goes
    straight to RECONCILED, matching the "no manual ops step for PhonePe"
    product decision. Idempotent by the same status guard: the webhook and
    the customer's browser return can both call this for the same
    transaction (whichever arrives first wins), and RECONCILED itself is
    also excluded so a second call is a safe no-op."""
    if txn.status in ("INITIATED", "QR_GENERATED", "GATEWAY_SUCCESS"):
        txn.utr = (transaction_id or "").strip()[:60]
