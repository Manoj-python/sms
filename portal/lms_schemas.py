...
class LoanSummary(TolerantModel):
    ...
    # Confirmed live 2026-07-22 (loan L2WNTSGDW-240710121): the penal
    # interest RATE (percent per month) applied to a late installment's
    # full DueAmount for however many days it was actually paid late —
    # NOT the same as lpi_dues/RepaymentScheduleEntry.lpc, which reflect a
    # different (much smaller) AllCloud-internal LPC computation. Used by
    # foreclosure_statement_pdf.compute_foreclosure to reproduce AllCloud's
    # own back-office "Generated" LPI figure.
    lpc_interest_pct: float = Field(default=0.0, validation_alias=AliasChoices("LPCInterest",))
