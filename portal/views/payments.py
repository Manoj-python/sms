...
from portal.services import payment_service, phonepe_client
...
def _pay_error_ctx(loan, finance_id: str, error_key: str, selected_option: str, part_amount_value):
    """Shared by every "re-render the pay form with an error" branch below
    (bad amount, AllCloud GetQRCode down, PhonePe order-creation down) —
    all need the same recomputed totals/floors, just with a different
    error_key. See pay_form.html's error_key branches."""
    ...
    ctx = {
        ...
        "total_due_display": total, "selected_option": selected_option,
        "part_amount_value": part_amount_value,
    }
...
async def _create_pending_txn(request, sess, finance_id: str, lender: str, lms, gateway: str):
    """Shared prefix for generate_qr and generate_phonepe_order: re-fetch
    the loan, validate the customer's chosen amount, split into
    principal/lpi/collection components, and create the PgTransaction.
    Returns (txn, loan, option, None) on success, or
    (None, loan, option, error_response) if the amount was invalid — the
    caller should return error_response as-is in that case."""
    ...
    try:
        ...
        ctx = _pay_error_ctx(loan, finance_id, str(exc), option, part_amount)
        return None, loan, option, render(request, "partials/pay_form.html", ctx)
    ...
    txn = await payment_service.create_transaction(
        ...
        agreement_no=loan.agreement_no if loan else "",
        amount=principal,
        lpi_amount=lpi,
    ...
    return txn, loan, option, None


@require_session
async def generate_qr(request, sess, finance_id: str):
    """HTMX: creates the pg_transaction and renders the customer's chosen
    payment method — Pay Now gateway button (AllCloud). See
    generate_phonepe_order for the alternative gateway."""
    await assert_loan_access(sess, finance_id, request)
    lender = sess.finance_lenders.get(str(finance_id), "smsquare")
    lms = get_lms(lender)
    ...
    txn, loan, option, error_response = await _create_pending_txn(
        request, sess, finance_id, lender, lms, gateway="allcloud",
    )
    ...
    try:
        qr = await lms.get_qr_code(
            finance_id=finance_id,
            due_amount=txn.amount,
            collection_charges=txn.collection_charges,
            lpi_amount=txn.lpi_amount,
            show_qr=show_qr,
        ...
        return render(request, "partials/pay_form.html",
                      _pay_error_ctx(loan, finance_id, "err_lms_down", option, request.POST.get("part_amount")))
    ...
    await audit(request, "qr_generated",
                detail=f"txn={txn.id} finance_id={finance_id} agreement_no={loan.agreement_no if loan else ''} "
                       f"amount={txn.total_amount} gateway=allcloud method={method}",
                session_id=sess.id, mobile_mask=sess.mobile_mask, mobile=sess.mobile)
    return render(request, "partials/payment_result.html", {"txn": txn, "qr": qr})


@require_session
async def generate_phonepe_order(request, sess, finance_id: str):
    """HTMX: same amount-selection flow as generate_qr, but routes the
    customer to PhonePe's Standard Checkout instead of AllCloud's GetQRCode.
    Unlike AllCloud, completion is only marked once PhonePe genuinely
    confirms it — see phonepe_return (customer's browser lands there after
    checkout) and phonepe_webhook (PhonePe's own server-to-server
    confirmation) — this view only creates the pending transaction and
    redirects."""
    if not get_settings().phonepe_enabled:
    ...
    txn, loan, option, error_response = await _create_pending_txn(
        request, sess, finance_id, lender, lms, gateway="phonepe",
    )
    if error_response is not None:
        return error_response

    redirect_url = f"{get_settings().portal_base_url}/loan/{finance_id}/pay/phonepe/return/{txn.idempotency_key}"
    try:
        checkout_url, order_id = await phonepe_client.create_order(
            txn.idempotency_key, float(txn.total_amount), redirect_url,
        )
    except phonepe_client.PhonePeError:
        txn.status = "FAILED"
        txn.last_error = "PhonePe create_order failed"
        await txn.asave()
        return render(request, "partials/pay_form.html",
                      _pay_error_ctx(loan, finance_id, "err_lms_down", option, request.POST.get("part_amount")))

    txn.lms_receipt_ref = order_id[:80]
    await txn.asave()
    await audit(request, "qr_generated",
                detail=f"txn={txn.id} finance_id={finance_id} agreement_no={loan.agreement_no if loan else ''} "
                       f"amount={txn.total_amount} gateway=phonepe",
                session_id=sess.id, mobile_mask=sess.mobile_mask, mobile=sess.mobile)
    return render(request, "partials/payment_result.html", {"txn": txn, "phonepe_redirect_url": checkout_url})


@require_session
async def phonepe_return(request, sess, txn_key: str):
    """Customer's browser lands here after PhonePe's checkout page. This is
    NOT an authenticated server-to-server confirmation (PhonePe didn't call
    this — the customer's browser did, which could in principle hit this
    URL without having paid), so the actual payment state is always
    re-verified against PhonePe's own status API before marking anything
    RECONCILED — never trust the redirect alone. phonepe_webhook is the
    other, authoritative path to the same end state; whichever lands first
    wins (confirm_phonepe_payment is idempotent)."""
    if not get_settings().phonepe_enabled:
        return HttpResponseNotFound()
    txn = await payment_service.get_owned_txn_by_key(txn_key, sess.id)
    if txn is None:
        return HttpResponseNotFound()
    if txn.status != "RECONCILED":
        try:
            state = await phonepe_client.check_status(txn_key)
        except phonepe_client.PhonePeError:
            state = "PENDING"
        if state == "COMPLETED":
            txn = await payment_service.confirm_phonepe_payment(
                txn, await phonepe_client.transaction_id_for(txn_key),
            )
        elif state == "FAILED" and txn.status != "RECONCILED":
            txn.status = "FAILED"
            txn.last_error = "PhonePe order FAILED"
            await txn.asave()
    return render(request, "partials/payment_result.html", {"txn": txn, "qr": None})


async def phonepe_webhook(request):
    """PhonePe's server-to-server payment confirmation — no portal session
    exists here (this isn't a customer request), so ownership is not the
    gate: phonepe_client.validate_webhook's own X-VERIFY checksum check
    (same salt-key signing as every outbound request) is what authorizes
    this. Always returns 200 (even on a validation failure or unknown
    order) so PhonePe doesn't retry-storm this endpoint; the
    customer-facing phonepe_return path is the fallback if this webhook is
    ...
    try:
        callback = phonepe_client.validate_webhook(auth_header, raw_body)
    except phonepe_client.PhonePeError:
        return HttpResponse(status=200)

    merchant_order_id = callback["merchant_order_id"]
    if not merchant_order_id:
        return HttpResponse(status=200)

    txn = await payment_service.get_txn_by_key(merchant_order_id)
    if txn is None:
        return HttpResponse(status=200)

    if callback["state"] == "COMPLETED":
        await payment_service.confirm_phonepe_payment(txn, callback["transaction_id"])
    elif callback["state"] == "FAILED" and txn.status != "RECONCILED":
        txn.status = "FAILED"
        txn.last_error = "PhonePe webhook FAILED"
        await txn.asave()
    return HttpResponse(status=200)
...
async def receipt_pdf(request, sess, txn_id: int):
    ...
    await audit(request, "receipt_downloaded",
                detail=f"txn={txn.id} finance_id={txn.finance_id} agreement_no={txn.agreement_no}",
                session_id=sess.id, mobile_mask=sess.mobile_mask, mobile=sess.mobile)
    return HttpResponse(
        content=pdf,
        content_type="application/pdf",
...
@require_session
async def generate_foreclosure_qr(request, sess, finance_id: str):
    """HTMX: AllCloud checkout link for the FULL foreclosure amount shown on
    foreclosure_view.html — no amount selection (unlike generate_qr's
    regular-dues flow), since the whole point of this button is to pay off
    exactly what's owed to close the loan. Recomputes compute_foreclosure
    fresh here rather than trusting whatever the page happened to show on
    load — same "never trust the form" posture as generate_qr, and doubly
    important here since LPI/broken-interest genuinely change day to day."""
    await assert_loan_access(sess, finance_id, request)
    lender = sess.finance_lenders.get(str(finance_id), "smsquare")
    lms = get_lms(lender)
    try:
    ...
    gated = await _seize_gate(request, sess, finance_id, loan, lcc, "generate_foreclosure_qr")
    if gated is not None:
        return gated
    gated = await _foreclosure_dues_gate(request, sess, finance_id, loan, "generate_foreclosure_qr")
    if gated is not None:
        return gated

    calc = compute_foreclosure(loan, lcc)
    # Same due/lpi/collection split concept as generate_qr's regular-dues
    # flow (see get_qr_code) — LPI and VAS map directly onto their existing
    # buckets; everything else foreclosure-specific (fore-closure charges,
    # future principal, broken interest, EMI overdue, handloan, insurance)
    # has no dedicated bucket on the gateway call, so it's lumped into
    # due_amount. Only affects what AllCloud's checkout page displays as
    # the breakdown — the actual TotalAmount charged is unaffected.
    lpi_amount = calc["lpi"]
    collection_charges = calc["vas_charge"]
    due_amount = round(calc["total_receivables"] - lpi_amount - collection_charges, 2)

    txn = await payment_service.create_transaction(
        session_id=sess.id, mobile=sess.mobile, finance_id=finance_id,
        agreement_no=loan.agreement_no, amount=due_amount, lpi_amount=lpi_amount,
        collection_charges=collection_charges, payment_option="foreclosure", lender=lender,
    )
    try:
        qr = await lms.get_qr_code(
            finance_id=finance_id, due_amount=due_amount, collection_charges=collection_charges,
            lpi_amount=lpi_amount, show_qr=False, sms_link=False,
        )
    except LMSError:
        txn.status = "FAILED"
        txn.last_error = "GetQRCode failed"
    ...
    txn.lms_receipt_ref = qr.reference[:80]
    await txn.asave()
    # Same optimistic-terminal-state posture as generate_qr — no server-to-
    # server callback from AllCloud; ops reconciles into AllCloud out-of-band.
    txn = await payment_service.confirm_gateway_payment(txn, utr="")
    await audit(request, "qr_generated",
                detail=f"txn={txn.id} finance_id={finance_id} agreement_no={loan.agreement_no} "
                       f"amount={txn.total_amount} gateway=allcloud method=foreclosure",
                session_id=sess.id, mobile_mask=sess.mobile_mask, mobile=sess.mobile)
    return render(request, "partials/payment_result.html", {"txn": txn, "qr": qr})
...
async def payment_receipt_view(request, sess, txn_id: int):
    ...
    if txn is None or txn.status != "RECONCILED":
        return HttpResponseNotFound()
    await audit(request, "receipt_viewed",
                detail=f"txn={txn.id} finance_id={txn.finance_id} agreement_no={txn.agreement_no}",
                session_id=sess.id, mobile_mask=sess.mobile_mask, mobile=sess.mobile)
